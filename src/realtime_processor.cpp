...
void Biquad::configure(Type type, float cutoffHz, float sampleRate, float q) {
...
Processor::Processor(const Options& options)
    : rate_(options.rate), channels_(options.channels), gain_(options.gain),
      lowPassHz_(options.lowPassHz), highPassHz_(options.highPassHz),
      noiseGateDb_(options.noiseGateDb), routing_(options.routing),
      lowPass_{Biquad(options.channels), Biquad(options.channels)},
      highPass_{Biquad(options.channels), Biquad(options.channels)} {
    update_filters();
}
...
void Processor::set_bypass(bool value) {
    std::lock_guard<std::mutex> lock(mutex_);
    if(bypass_ && !value) update_filters();
    bypass_ = value;
}
...
void Processor::set_low_pass(float value) {
    std::lock_guard<std::mutex> lock(mutex_);
    lowPassHz_ = value;
    update_filters();
}

void Processor::set_high_pass(float value) {
    std::lock_guard<std::mutex> lock(mutex_);
    highPassHz_ = value;
    update_filters();
}
